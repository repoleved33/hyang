...
interface AddMyShelfModalProps {
  visible: boolean;
  onClose: () => void;
  mainPerfumes: Perfume[];
  myPerfumesPerfId: string[]; // 이미 추가된 perfId 리스트
  addMyPerfume: (perfume: Perfume) => void;
}

export default function AddMyShelfModal({
  visible,
  onClose,
  mainPerfumes,
  myPerfumesPerfId,
  addMyPerfume,
}: AddMyShelfModalProps) {
  const [searchKeyword, setSearchKeyword] = useState("");

  const keyword = searchKeyword.trim().toLowerCase();

  const filteredMainPerfumes = mainPerfumes.filter((p) => {
    if (myPerfumesPerfId.includes(p.perfId)) return false;
